...
function Get-LatestVersion {
    ...
    try {
        ...
        # Extract version for 'clawne-me' from lines like: name:version
        $line = ($content -split "`n") | Where-Object { $_ -match '^\s*clawne-me\s*:' } | Select-Object -First 1
        if (-not $line) {
            Write-Host "No version found for clawne-me, falling back to default: $DefaultVersion"
            return $DefaultVersion
        }
        ...
        Write-Host "Latest clawne-me version: $rawVersion"
        return $rawVersion
    } catch {
...
function Confirm-VersionExists {
    ...
    try {
        Invoke-RestMethod -Uri $ApiUrl -UseBasicParsing -Headers @{ "User-Agent" = "clawne-me-installer" } | Out-Null
    } catch {
        $StatusCode = $null
...
function Install-Binary {
    ...
    # Install binary
    $InstallDir = "$env:LOCALAPPDATA\Programs\clawne-me"
...
function Initialize-Environment {
    $ClawneMeHome = "$env:USERPROFILE\.clawne-me"

    if (-not (Test-Path $ClawneMeHome)) {
        New-Item -ItemType Directory -Path $ClawneMeHome | Out-Null
    }
    
    $Folders = @("config", "specs", "src", "certs")
    foreach ($Folder in $Folders) {
        $Path = Join-Path $ClawneMeHome $Folder
        if (-not (Test-Path $Path)) {
            New-Item -ItemType Directory -Path $Path | Out-Null
        }
    }

    Write-Host "Created clawne-me environment at $ClawneMeHome"
}
