...
# If no version specified, fetch the latest version of clawne-me
if [[ -z "$VERSION" ]]; then
...
setup_env() {
  CLAWNE_ME_HOME="$HOME/.clawne-me"
  mkdir -p "$CLAWNE_ME_HOME/config" "$CLAWNE_ME_HOME/specs" "$CLAWNE_ME_HOME/src" "$CLAWNE_ME_HOME/certs"
  echo "Created clawne-me environment at $CLAWNE_ME_HOME"
}
